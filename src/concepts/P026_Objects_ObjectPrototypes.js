...
//    JavaScript provides to do this is messy and complex, but it can be significantly simplified.
//    Example: Let's add a create method to the Object function. The create method creates a new object that uses an
...
// 6. The prototype link has no effect on object modification. When an object is modified, the object's  prototype is
//    left touched.
...
// 7. Delegation
//    The prototype link is used ONLY in retrieval. If we try to retrieve a property value from an object, and if
//    the object lacks the property name, then JavaScript attempts to retrieve the property value from the prototype
//    object. And if that object is lacking the property, then it goes to its prototype, and so on until the process
//    finally bottoms out with Object.prototype. If the desired property exists nowhere in the prototype chain, then
//    the result is the undefined value. This is called delegation.

// 8. The prototype relationship is a dynamic relationship. If we add a new property to a prototype, that property
