...
// 2. If one of the operand for + operator is a string, then string concatenation will occur instead of addition.
name = name + 3;
...
// Bitwise Operator
// 1. It is strange to have bitwise operators in a language like JavaScript which does not have Integer data type.
// 2. The bitwise operator in JavaScript operates by converting the operand to a 32-bit signed integer, performing
//    bitwise operation on the operands and then converting the result back into a 64-bit bit floating point number.
// 3. JavaScript bitwise computations are slow because of these to and fro conversions from floating point number.
//    Use bitwise operator only for bitwise computation and not for a faster computation as done in C.
var a = 5;
var b = 13;
result = ~a; // -6. NOT - Invert all the bits in a to get the 'result'
result = a | b; // 13. OR - The bit in 'result' will be 1 if the corresponding bit is 1 in either a or b
result = a & b; // 5. AND - The bit in 'result' will be 1 if the corresponding bit is 1 in both a and b
result = a ^ b; // 8. XOR - The bit in 'result' will be 1 if the corresponding bit is 1 in either a or b but not both.
result = a >> b; // 0. RIGHT SHIFT - Shift binary representation of 'a' by 'b' bits to the right & discard shifted bits.
result = a << b; // 40960. LEFT SHIFT - Shift binary representation of 'a' by 'b' bits to the right, filling zeros.
result = a >>> b; // 0. ZERO FILLED RIGHT SHIFT - Shift binary representation of 'a' by 'b' bits to the right, discarding
